...
This file is generated by the Improver agent after each monthly lesson review. Its structure is fixed so the pre-merge hook and human reviewer can parse it consistently.
...
**Target file**: `.github/[path/to/file]`
**Status**: PENDING | APPROVED | REJECTED | DEFERRED

**Rationale**:
[What recurring lesson or pattern justifies this change?]

**Proposed change**:
[Describe the change in plain language. Do NOT paste protected section content inline.]

**Risk level**: low | medium | high
**Protected sections affected**: none | [list section tags if any]
...
|------|----------|----------|-------|
| YYYY-MM-DD | [short-title] | APPROVED / REJECTED / DEFERRED | [brief rationale] |
```

### Rules for the Improver when writing proposals

- Never paste `<!-- PROTECTED: ... -->` block content into `PROPOSED_CHANGES.md`
- Set **Protected sections affected** to `none` if the change does not touch any protected block
- List the protected section **tag name only** (not content) if a protected block is involved
- Do not include more than one proposal per `###` heading
- Keep rationale factual: cite the lesson entity names from the knowledge graph that motivate the change
