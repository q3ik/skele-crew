# Marketing Draft — Social Post With Metric Claim

> Fixture for Marketing→Lawyer peer review test.
> This draft contains a percentage/accuracy claim and MUST trigger Lawyer review.

---

## Draft Post

Buzzy Game keeps kids spelling sharp — with **99% spelling accuracy** across all grade levels.
Try it free today 🐝 buzzy-game.pages.dev

---

## Metadata

- **Platform**: Bluesky
- **Product**: buzzy-game
- **Contains metric claim**: yes — "99% spelling accuracy"
- **Lawyer review required**: yes (Pre-Publish Checklist: percentage/accuracy claims → Lawyer review)
